...
function renderEventDetail(ev) {
    ...
    const groupTopHtml = groupName
        ? `<div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">
               ${groupIconUrl ? `<img src="${imgThumb(groupIconUrl, 64)}" style="width:16px;height:16px;border-radius:3px;object-fit:cover;flex-shrink:0;">` : `<span class="msi" style="font-size:14px;color:var(--tx2);">group</span>`}
    ...
    const groupHtml = groupName
        ? `<div class="fd-section-label" style="margin-top:12px;">${t('calendar.detail.organizer', 'Organizer')}</div>
           <div style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:6px 0;" onclick="navOpenModal('group','${groupOpenId}','${jsq(groupName || '')}')">
               ${groupIconUrl ? `<img src="${imgThumb(groupIconUrl, 64)}" style="width:28px;height:28px;border-radius:6px;object-fit:cover;">` : ''}
