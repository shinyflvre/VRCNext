...
function _miAuthorHtml(authorId, authorName) {
    ...
    const badge = authorId
        ? `<span onclick="navOpenModal('friend','${jsq(authorId)}','${jsq(authorName)}')" style="display:inline-flex;align-items:center;padding:1px 8px;border-radius:20px;background:var(--badge-bg);font-size:calc(11px + var(--fs-off, 0px));font-weight:600;color:var(--badge-tx);cursor:pointer;line-height:1.8;">${esc(authorName)}</span>`
        : esc(authorName);
