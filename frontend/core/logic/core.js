...
function getProfileUsernames(raw) {
    const out = { username: '', previous: [] };
    if (!raw || typeof raw !== 'object') return out;
    out.username = typeof raw.username === 'string' ? raw.username.trim() : '';
    const cur = String(raw.displayName || '').trim().toLowerCase();
    const seen = new Set([out.username.toLowerCase()]);
    const past = Array.isArray(raw.pastDisplayNames) ? raw.pastDisplayNames.slice() : [];
    past.sort((a, b) => String(b?.updated_at || '').localeCompare(String(a?.updated_at || '')));
    past.forEach(p => {
        const n = String(p?.displayName || '').trim();
        const k = n.toLowerCase();
        if (!n || k === cur || seen.has(k)) return;
        seen.add(k);
        out.previous.push(n);
    });
    return out;
...
function getProfileUsernameRows(raw, row) {
    const u = getProfileUsernames(raw);
    const rows = [];
    if (u.username) rows.push(row(t('profiles.meta.username', 'Username'), esc(u.username)));
    if (u.previous.length) rows.push(row(t('profiles.meta.previous_username', 'Previous Username'), esc(u.previous.join(', '))));
    return rows;
}
...
const _TE_GROUPS = [
    ...
    { title: 'Buttons', vars: [
        ...
        ['accent', 'Accent'], ['accent-lt', 'Accent Light'], ['dropdown-active', 'Dropdown Active'], ['cyan', 'Highlight'],
    ]},
    { title: 'Border', vars: [
