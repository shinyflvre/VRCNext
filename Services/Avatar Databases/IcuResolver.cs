...
public sealed class IcuResolver
{
    public const int MaxBatch = 80;

    private const string Endpoint      = "https://avtr.icu/resolve";
    ...
    public IcuResolver(Action<string>? log = null) => _log = log;
    ...
    public async Task<JObject?> ResolveDirectAsync(string fileId)
    {
        ...
        catch (Exception ex) { _log?.Invoke($"icu resolve direct failed: {ex.Message}"); return null; }
    }
    ...
    private async Task DrainAsync()
    {
        while (true)
        {
            ...
            {
                _log?.Invoke($"icu resolve failed: {ex.Message}");
    ...
    private async Task<Dictionary<string, JObject?>> PostBatchAsync(List<string> fileIds)
    {
        ...
        var payload = JsonConvert.SerializeObject(new { ids = fileIds });
    ...
    private async Task<string> SendAsync(HttpClient client, string payload)
    {
        ...
        Log?.Invoke($"[ICU] QRY resolve x{_lastBatchSize}");
        var resp = await client.SendAsync(req);
        var text = await resp.Content.ReadAsStringAsync();

        if (!resp.IsSuccessStatusCode)
        {
            _log?.Invoke($"icu resolve [{(int)resp.StatusCode}]: {text[..Math.Min(160, text.Length)]}");
            return "";
        }
